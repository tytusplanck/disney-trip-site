...
describe('trip root redirect', () => {
  it('redirects the base trip path to the first planner section', () => {
    expect(source.includes('import { findTripDataModule, findTripSummary, getTripLandingPath }')).toBe(true);
    expect(source.includes("const tripSlug = Astro.params['trip'] ?? '';")).toBe(true);
    expect(source.includes('const trip = findTripSummary(allTripsData.trips, tripSlug);')).toBe(true);
    expect(source.includes('const tripLandingPath = getTripLandingPath(trip, tripModule);')).toBe(true);
