...
This project is intentionally deployed as a server-rendered Astro app because the site key gate depends on server-side checks and signed cookies.
...
The env var name stays `SITE_PASSWORD` for deployment compatibility, even though the product copy now calls it a site key.

If preview deployments should be isolated from production, use different site key values for each environment.
...
Astro's built-in `security.checkOrigin` setting is disabled in this project. The deployed Vercel form flow can trip Astro's default SSR origin comparison and block the shared site key login form before the endpoint code runs.

That means this project currently depends on its own application-level protections instead:

- site key check on the server
...
The site key gate protects the deployed site only. It does not hide source code in GitHub.
...
2. Entering the correct site key should return access to `/`.
